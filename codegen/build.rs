...
static COLUMNS: phf::Map<&'static str, (&'static str, &'static str, &'static str)> = phf_map! {
    ...
    "modulus/young" => ("youngs_modulus", "Pressure", "gigapascal"),
    "modulus/rigidity" => ("rigidity_modulus", "Pressure", "gigapascal"),
...
fn main() -> Result<(), Box<dyn StdError>> {
    ...
    let ptable = match data {
        Value::Object(map) => match map.get("pTable").ok_or(Error::missing_field("pTable")) {
...
pub enum Element {
    ...
    fn make_function(
        ...
        for val in values {
            ...
            let (data_name, inner_val) = match data_name_parts.as_slice() {
                [outer, inner] => (
                    ...
                    match val.get(outer) {
                        ...
                        None => {
                            if inner == &"mohs"
                                && val.get("name") == Some(&Value::String("Boron".into()))
                            {
                                dbg!(&val, outer);
                                panic!("uh");
                            }
            ...
            if type_name.starts_with("Vec<") {
                ...
                match inner_val.get(data_name).map(|f| f.is_array()) {
                    Some(true) => {}
                    Some(false) => {
                        dbg!(data_name, inner_val.get(data_name).expect("crap"));
                    }
                    None => {
                        dbg!(&inner_val, data_name);
                    }
                }
        ...
        let func_body: Vec<String> = match_map
            .into_iter()
            .map(|(k, v)| format!("Self::{} => {}", k, v))
            .collect();
        ...
        format!(
            r#"
/// Get this constant for the element.
pub fn {}(&self) -> {} {{
    match self {{
        {}
    }}
...
"#,
    ...
    let impl_body: String = COLUMNS
        .into_iter()
        .map(|(&data, &(field, type_, unit))| {
