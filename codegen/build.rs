...
static COLUMNS: phf::Map<&'static str, (&'static str, &'static str, &'static str)> = phf_map! {
    ...
    "boiling_point/kelvin" => ("boiling_point", "ThermodynamicTemperature", "kelvin"),
    "melting_point/kelvin" => ("melting_point", "ThermodynamicTemperature", "kelvin"),
...
fn main() -> Result<(), Box<dyn StdError>> {    
    ...
    let dest_path = Path::new(&out_dir).join("element_base.rs");
            ...
            if type_name.starts_with("Vec<") {
                ...
                let val_constructor = match type_name.chars().next().unwrap() {
                    'u' | 'i' => {
                        // numeric type, string conversion is fine and type inference will
                        // handle the rest
                        display_opt(val.get(data_name))
                    },
                    'f' => {
                        // have to add dot if not there
                        display_opt(val.get(data_name).map(floatify))
                    },
                    _ => {
                        // need to construct type ourselves
                        let f_val = val.get(data_name).map(|v| {
                            let fv = floatify(v);
